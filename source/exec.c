...
ZORAVM_ERROR ZoraVME_Push(ZoraVM *vm, ZoraVM_Data data) {
  ...
  case ZORASM_DATA_INT:
    vm->mem[vm->mp] = (ZoraVM_Data){ZORASM_DATA_INT, .val.integer = data.val.integer};
    break;
  case ZORASM_DATA_FLOAT:
    vm->mem[vm->mp] = (ZoraVM_Data){ZORASM_DATA_FLOAT, .val.floating = data.val.floating};
    break;
  case ZORASM_DATA_STRING:
    vm->mem[vm->mp] = (ZoraVM_Data){ZORASM_DATA_STRING, .val.string = data.val.string};
    break;
  default:
...
ZORAVM_ERROR ZoraVME_Add(ZoraVM *vm) {
  ...
  if (one.kind == ZORASM_DATA_INT && two.kind == ZORASM_DATA_INT) {
    vm->mem[vm->mp - 2].val.integer += two.val.integer;
    vm->mem[vm->mp - 2].kind = ZORASM_DATA_INT;
  } else if (one.kind == ZORASM_DATA_STRING && two.kind == ZORASM_DATA_STRING) {
    vm->mem[vm->mp - 2].val.string = strcat(vm->mem[vm->mp - 2].val.string, two.val.string);
    vm->mem[vm->mp - 2].kind = ZORASM_DATA_STRING;
  } else if (one.kind == ZORASM_DATA_FLOAT && two.kind == ZORASM_DATA_FLOAT) {
...
ZORAVM_ERROR ZoraVME_Inc(ZoraVM *vm) {
  ...
  else if (vm->mem[vm->mp - 1].kind == ZORASM_DATA_FLOAT)
    ++vm->mem[vm->mp - 1].val.floating;
  else if (vm->mem[vm->mp - 1].kind == ZORASM_DATA_STRING)
...
ZORAVM_ERROR ZoraVME_Factorial(ZoraVM *vm) {
  ...
  if (data.kind != ZORASM_DATA_INT)
    return ZORAVM_ERROR_ILLEGAL_INST_TYPE;

  vm->mem[vm->mp - 1].val.integer = ZORA_M_factorial(data.val.integer);
  vm->mem[vm->mp - 1].kind = ZORASM_DATA_INT;
...
ZORAVM_ERROR ZoraVME_Neg(ZoraVM *vm) {
  if (vm->mp < 1)
    return ZORAVM_ERROR_MEMORY_EMPTY;
  if (vm->mem[vm->mp - 1].kind == ZORASM_DATA_STRING)
    return ZORAVM_ERROR_UNEXPECTED_TYPE;

  if (vm->mem[vm->mp - 1].kind == ZORASM_DATA_FLOAT)
    vm->mem[vm->mp - 1].val.floating = -vm->mem[vm->mp - 1].val.floating;
  else if (vm->mem[vm->mp - 1].kind == ZORASM_DATA_INT)
    vm->mem[vm->mp - 1].val.integer = -vm->mem[vm->mp - 1].val.integer;
  vm->ip++;
...
ZORAVM_ERROR ZoraVME_JmpIf(ZoraVM *vm, ZoraVM_Program prog) {
  ...
  if (vm->mem[vm->mp - 1].kind != ZORASM_DATA_INT) return ZORAVM_ERROR_ILLEGAL_INST;
...
ZORAVM_ERROR ZoraVME_Scan(ZoraVM *vm) {
  ...
  ZoraVM_Data data = {.kind = ZORASM_DATA_STRING, };
...
ZORAVM_ERROR ZoraVME_Env(ZoraVM* vm) {
  ...
  if (data.kind != ZORASM_DATA_STRING) return ZORAVM_ERROR_UNKNOWN_TYPE;

  vm->mem[vm->mp - 1] = (ZoraVM_Data) {
    .kind = ZORASM_DATA_STRING,
...
ZORAVM_ERROR ZoraVME_Ret(ZoraVM *vm, ZoraVM_Program prog) {
  vm->mem[vm->mp].kind = ZORASM_DATA_INT;
...
ZORAVM_ERROR ZoraVME_Dump_Stack(ZoraVM *vm) {
  ...
  for (int i = 0; i < vm->sp; i++) {
    ...
    switch (entry.kind) {
    case ZORASM_DATA_INT:
      printf("\t %d: \t %lld \t [%s]\n", i, entry.val.integer, Zorasm_Datakind[entry.kind]);
      break;
    case ZORASM_DATA_FLOAT:
      printf("\t %d: \t %lf \t [%s]\n", i, entry.val.floating, Zorasm_Datakind[entry.kind]);
      break;
    case ZORASM_DATA_STRING:
      printf("\t %d: \t %s \t [%s]\n", i, entry.val.string, Zorasm_Datakind[entry.kind]);
      break;
    default:
      return ZORAVM_ERROR_UNKNOWN_TYPE;
    }
